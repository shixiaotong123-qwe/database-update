...
/// 清空现有数据
async fn clear_existing_data(pool: &PgPool) -> Result<()> {
    info!("清空现有数据...");
    ...
    // 重置序列
    sqlx::query("ALTER SEQUENCE users_id_seq RESTART WITH 1").execute(pool).await
        .context("重置users序列失败")?;
    sqlx::query("ALTER SEQUENCE products_id_seq RESTART WITH 1").execute(pool).await
        .context("重置products序列失败")?;
    sqlx::query("ALTER SEQUENCE orders_id_seq RESTART WITH 1").execute(pool).await
...
/// 插入用户数据
async fn insert_users(pool: &PgPool) -> Result<()> {
    info!("插入用户数据...");
...
async fn insert_products(pool: &PgPool) -> Result<()> {
    ...
    let products = vec![
        ("iPhone 15 Pro", "苹果最新旗舰手机", "8999.00", 1, 50, "IPH15PRO001"),
        ("MacBook Air M2", "轻薄笔记本电脑", "7999.00", 1, 30, "MBA-M2-001"),
        ("iPad Pro 11", "专业平板电脑", "5999.00", 1, 25, "IPADPRO11-001"),
        ("AirPods Pro", "降噪无线耳机", "1599.00", 2, 100, "APP-001"),
        ("Apple Watch Series 9", "智能手表", "2999.00", 2, 75, "AWS9-001"),
        ("小米13 Ultra", "小米旗舰手机", "4999.00", 1, 40, "MI13U-001"),
        ("华为MateBook X", "轻薄商务笔记本", "6999.00", 1, 20, "HMBX-001"),
        ("Sony WH-1000XM5", "降噪头戴耳机", "2399.00", 2, 60, "SONY-WH5-001"),
        ("戴尔显示器27寸", "4K专业显示器", "2199.00", 3, 35, "DELL-27-4K"),
        ("机械键盘", "Cherry轴游戏键盘", "599.00", 3, 80, "MECH-KB-001"),
    ];
    
    for (name, description, price_str, category_id, stock, sku) in products {
        sqlx::query(
            "INSERT INTO products (product_name, description, price, category_id, stock_quantity, sku, is_active) 
...
/// 插入订单数据
async fn insert_orders(pool: &PgPool) -> Result<()> {
    info!("插入订单数据...");
...
/// 显示插入的数据统计
pub async fn show_data_statistics(pool: &PgPool) -> Result<()> {
    info!("数据统计信息:");
...
/// 获取高级统计信息
pub async fn show_advanced_statistics(pool: &PgPool) -> Result<()> {
    info!("高级统计信息:");
    
    // 用户订单统计
    let user_order_stats = sqlx::query(
        "SELECT 
            u.full_name,
